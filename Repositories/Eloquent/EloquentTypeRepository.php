...
class EloquentTypeRepository extends EloquentBaseRepository implements TypeRepository
{
        ...
        if ($filter) {
            ...
            if (isset($filter->parentSlug) && is_array($filter->parentSlug)) {
                $query->whereIn('parent_id', function ($query) use ($filter) {
                    $query->select('iblog__types.id')
                        ->from('iblog__types')
                        ->whereIn('iblog__types.slug', $filter->parentSlug);
                });
            }

            //Filter excluding types by ID
            if (isset($filter->excludeById) && is_array($filter->excludeById)) {
                $query->whereNotIn('id', $filter->excludeById);
            }

            //Get specific types by ID
    ...
    public function create($data)
    {
       // dd($data);
        ...
        event(new TypeWasCreated($type, $data));
