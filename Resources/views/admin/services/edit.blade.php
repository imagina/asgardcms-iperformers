...
@section('content-header')
    <h1>
        {{ trans('iperformers::services.title.edit service') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.iperformers.service.index') }}">{{ trans('iperformers::services.title.services') }}</a></li>
        <li class="active">{{ trans('iperformers::services.title.edit service') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.iperformers.service.update', $service->id], 'method' => 'put']) !!}
    <div class="row">
                            ...
                            <div class="tab-content">
                                ...
                                @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                                    ...
                                    <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                                        @include('iperformers::admin.services.partials.edit-fields', ['lang' => $locale])
                                    </div>
                                @endforeach
                ...
                <div class="col-xs-12 ">
                    <div class="box box-primary">
                        <div class="box-header">
                            ...
                            <div class="box-body">
                                @include('iperformers::admin.fields.image',['entity'=>$service])
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            $(document).keypressAction({
                actions: [
                    { key: 'b', route: "<?= route('admin.iperformers.service.index') ?>" }
                ]
            });
        });
    </script>
